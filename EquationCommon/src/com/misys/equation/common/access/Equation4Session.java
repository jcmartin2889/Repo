...
public class Equation4Session extends AbsEquationStandardSession
{
	// This attribute is used to store cvs version information.
	public static final String _revision = "$Id: Equation4Session.java 15717 2013-04-30 15:46:31Z whittap1 $";

	private static final EquationLogger LOG = new EquationLogger(Equation4Session.class);
	 ...
	 * @throws EQException
	 */
	public Equation4Session(EquationUser user, EquationConnectionWrapper connectionWrapper) throws EQException
	{
	 ...
	 * @throws EQException
	 */
	public Equation4Session(EquationUnit unit) throws EQException
	{
		super(unit);
	}
	...
	protected EquationStandardTransaction callX56HMR(EquationStandardTransaction transaction, String mode) throws EQException
	{
		...
		// API successfully applied? Note: GZREC_WARNING=Warnings (e.g. H15ARR), GZREC_TRANS_FWD_TELLER=Transaction forwarded
		if (rrec.equals(GZREC_SUCCESS) || rrec.equals(GZREC_WARNING) || rrec.equals(GZREC_TRANS_FWD_TELLER))
		{
			transaction.setGzBytes(gza);
			transaction.setValid(true);

			// for transaction forwarded by offline teller, mark messages as error message
			if (rrec.equals(GZREC_TRANS_FWD_TELLER))
			{
	...
	/**
	 * Call the UTW70C to print the desktop transaction journal
	 * 
	 * @param fro
	 *            - Program root
	 * @param wsid
	 ...
	 * @param dpost
	 *            - Print generated postings (hardcoded)
	 * @param newpg
	 *            - Start new page (hardcoded)
	 * @param supss
	 *            - Suppress settlement instructions (hardcoded)
	 * @param prnb
	 *            - Print lines even if empty (hardcoded)
	 * @param opid
	 *            - User Defined Option Being Journalled
	 */
	public boolean callUTW70C(String fro, String wsid, int dim, int tim, int seq, String usid, String ttp, String dpost,
					String newpg, String supss, String prnb, String opid) throws EQException
	{
		CallableStatement equationJournalPrint = null;

		try
		{
			// Get us the journal print API
			equationJournalPrint = connectionWrapper.getConnection().prepareCall(
							"{ CALL UTW70CPRC (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) }");

			// Set the parameter variables in the SQL procedure
			equationJournalPrint.setString(1, Toolbox.pad(fro, 10));
			equationJournalPrint.setString(2, Toolbox.pad(wsid, 4));
			equationJournalPrint.setInt(3, dim);
			equationJournalPrint.setInt(4, tim);
			equationJournalPrint.setInt(5, seq);
			equationJournalPrint.setString(6, Toolbox.pad(usid, 4));
			equationJournalPrint.setString(7, Toolbox.pad(ttp, 1));
			equationJournalPrint.setString(8, Toolbox.pad(dpost, 1));
			equationJournalPrint.setString(9, Toolbox.pad(newpg, 1));
			equationJournalPrint.setString(10, Toolbox.pad(supss, 1));
			equationJournalPrint.setString(11, Toolbox.pad(prnb, 1));
			equationJournalPrint.setString(12, Toolbox.pad(opid, 3));

			// Call the API
			equationJournalPrint.execute();
		}
		catch (SQLException e)
		{
			// Log the error but do not throw an exception
			LOG.error(e);
			LOG.error(LanguageResources.getFormattedString("Equation4Session.ErrorCallingProgram", "UTW70C") + ": "
							+ Toolbox.getExceptionMessage(e));
			return false;
		}
		finally
		{
			SQLToolbox.close(equationJournalPrint);
		}
		return true;
	}
	@Override
	...
	{
		// Check the WEYPF available on this session
		try
		{
			boolean isInTable = Toolbox.isColumnInTable("WEYPF", "WEYBDTA", this);
			return isInTable;
		}
		catch (EQException e)
		{
