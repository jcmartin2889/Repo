...
/**
 * This class is used in place of the standard EquationDataList, for the non-standard prompt/validate module HGR01R.
 * 
 * @author GoldsmC1
 * 
 */
public class EquationDataListHGR01R implements IDataList
{
	// This attribute is used to store cvs version information.
	public static final String _revision = "$Id: EquationDataListHGR01R.java 10671 2011-03-24 13:38:24Z lima12 $";

	private String dataList;
	private static final EquationLogger LOG = new EquationLogger(EquationDataListHGR01R.class);

	/**
	 * This is start of the SQL used by this PV.
	 * <p>
	 * The following details are effectively hard coded for HGR01R:<br>
	 * 1st DB file: HP10LF Limit control defaults by cntry/grp/cus<br>
	 * 2nd DB file: HG10LF Limit structure<br>
	 * files joined on: HPLSTR and HGLSTR
	 */

	private static final String SQL_BASE = "SELECT * FROM HP10LF INNER JOIN HG10LF ON HPLSTR = HGLSTR";

	/**
	 * Name of the primary file
	 */
	private static final String PRIMARY_FILE = "HP10LF";

	/**
	 * Name of field on HGPF for Limit Categories.
	 */
	private static final String CATEGORIES_FIELD = "HGLC";

	/**
	 * This field is a not a real key field.<br>
	 * (It is necessary to have this as a key field in the DS so that the PV works in Validate mode.)
	 */
	private static final String KEY_EXCEPTION_FIELD = "HPLSTR";

	/**
	 * Construct an empty data list
	 */
	public EquationDataListHGR01R()
	{
	}
					...
					int direction, int maxResults) throws EQException
	{
		...
		String sqlCategories = generateLimitCategoriesSQL(pvMetaData, equationPVData);
					...
					String start, EquationPVMetaData pvMetaData, EquationPVData equationPVData)
	{
		...
		try
		{
			...
			{
				limitCats = parseCategories(categoriesResultSet.getString(CATEGORIES_FIELD));

				for (String cat : limitCats)
				{
	...
	/**
	 * Generate the limit categories SQL
	 * 
	 * @param pvMetaData
	 *            - prompt validate meta data
	 * @param equationPVData
	 *            - prompt validate data
	 * @return - SQL statement to retrieve the limit categories
	 */
	private String generateLimitCategoriesSQL(EquationPVMetaData pvMetaData, EquationPVData equationPVData)
	{
		// sql statement
	...
	/**
	 * Parse the single Limit Categories field into an array.
	 * 
	 * @param categories
	 *            (single field)
	 * 
