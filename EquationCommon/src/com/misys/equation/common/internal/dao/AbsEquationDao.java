...
public abstract class AbsEquationDao extends SimpleJdbcDaoSupport implements IDao
{
	...
	public static final String _revision = "$Id: AbsEquationDao.java 13723 2012-07-02 09:55:17Z whittap1 $";

	private AbsRecord record;
	protected final EquationLogger LOG = new EquationLogger(this.getClass());

	// this attribute will ensure a unique database connection per dao.
	private SingleConnectionDataSource currentConnectionContainer;
	...
	{
		initialiseDao(connectionWrapper.getConnection());
	}
	...
	{
		List<AbsRecord> results = getRecordBy(whereClause);
	...
	{
		List<AbsRecord> results = getRecordBy(getWhereConditionBaseOnIdRecord());
		return createHashtableRecordModel(results);
	}
	...
	public int deleteRecordsBy(String whereClause)
	{
		...
		int count = updateTemplate.update(sqlBuilder.toString());
		if (isAutocommitable())
		{
			commit();
		}
	 ...
	 *             If more than one record was found in the database using the current record's primary key.
	 */
	public boolean reloadRecord()
	{
		// load the data models using the primary key from the database
		final List<AbsRecord> dataModels = getRecordBy(getWhereConditionBaseOnIdRecord());

		if (dataModels.size() > 1)
		{
			throw new IllegalStateException("More than one record returned in search by record ID!");
		}
		...
		setRecord(dataModels.get(0));

		// record successfully reloaded from database
		return true;
	}
	...
	public synchronized void insertRecords(final List<? extends AbsRecord> records)
	{
		...
		JdbcTemplate updateTemplate = getJdbcTemplate();
		StringBuilder sqlBuilder = new StringBuilder(1024);
		...
		String psSql = sqlBuilder.toString();

		if (LOG.isDebugEnabled())
		{
			LOG.debug("Executing batchUpdate: " + psSql + " for: " + records);
		}

		updateTemplate.batchUpdate(psSql, new BatchPreparedStatementSetter()
		{
			public void setValues(PreparedStatement ps, int i) throws SQLException
			{
				setRecord(records.get(i));
				int pi = 0;
				for (Object value : getParameterizedFieldsValues())
				{
					StatementCreatorUtils.setParameterValue(ps, ++pi, SqlTypeValue.TYPE_UNKNOWN, null, value);
				}
			}
		...
		// set record to null to prevent stale/inconsistent data
		setRecord(null);
	...
	/**
	 * Set the <code>AbsRecord</code> for this <code>AbsEquationDao</code>
	 * 
	 * @param record
	 *            - the record to set
	 */
	...
	/**
	 * @return true if auto-commit is set on the DAO, otherwise false
	 */
	...
	/**
	 * Set whether the DAO has auto-commit enabled
	 * 
	 * @param autocommitable
	 */
	...
	public class JournalStoreProcedure
	{
		...
		{
			String message = Toolbox.getExceptionMessage(exception);
