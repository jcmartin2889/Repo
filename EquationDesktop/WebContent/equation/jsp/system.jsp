...
<html:html>
	...
	<head>
		...
		<title><%=EquationCommonContext.getContext().getLanguageResource(equationUser,"GBLSYS")%></title>
		<script type="text/javascript">
			var RTL = <%=equationUser.isLanguageRTL()%>;
			var webFacUnit = '<%=equationLogin.getUnitId()%>';
			var webFacUser = '<%=equationLogin.getUserId()%>';
			var frameName = 'system';
			eqInit();		
		</script>
	</head>
	<BODY class="systemBody" 
	      <%=EqDesktopToolBox.getHTMLDirAttribute(equationUser.isLanguageRTL())%> 
	>
		<div id='systemDiv'>
		</div>
		<script type="text/javascript">
			if (RTL) setFieldRTL('systemDiv');
		</script>
	</BODY>
</html:html>
