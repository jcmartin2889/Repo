...
import com.misys.equation.common.ant.builder.version.EuqationVersionNumberDefinitions;
			...
			while (pos < line.length()) {
				...
				if (c == '\\') {
					if (pos == line.length()) {
						...
						element.append(c);
						element.append(EuqationVersionNumberDefinitions.LOCAL_LINE_BREAK.toString());
						line = reader.readLine();
						...
						switch (c) {		
							...
							case 'u':
								...
								break;
							default:
								element.append(previousCharacter);
								element.append(c);
								//break;
						}
					}
	...
	public void store(OutputStream out, String header) throws IOException
	{
		...
		for (int i=0; i<lineData.size(); i++) {
			...
			if (key.length() > 0) {  // This is a 'property' line, so rebuild it
				...
				if(value.contains(EuqationVersionNumberDefinitions.LOCAL_LINE_BREAK.toString())){
					lineValue=value.split(EuqationVersionNumberDefinitions.LOCAL_LINE_BREAK_REXP.toString());
					
					StringBuffer localBuffer=s;
					for (int index = 0; index < lineValue.length; index++) {
						
						formatForOutput (lineValue[index], localBuffer, false);
						writer.println (localBuffer);
